...
function(accounts_id  = getOption("rfacebookstat.accounts_id"), 
  ...
  if ( is.null(accounts_id) ) {
    ...
    message("...Loading apps from ", length(accounts_id), " account", ifelse( length(accounts_id) > 1, "s", "" ))
  ...
  for(account_id in accounts_id){
    ...
    QueryString <- paste0("https://graph.facebook.com/",api_version,"/",account_id,"/applications?fields=id,name,app_name,link,logo_url,created_time,description,app_type,category,subcategory,company&access_token=",access_token)
    ...
    answerobject <- content(answer, as = "parsed") # legacy fromJSON(answer)
